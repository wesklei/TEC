...
ifstream infile_complemento (FILE_OUT_COMPLEMENTO); //eh o out mesmo 
ofstream outfile_conjunto_independ (FILE_OUT_CONJUNTO_INDEP);
...
{ 
	...
	{ 
		if(found) break; 
		counter++; cout<<counter<<". "; outfile_conjunto_independ<<counter<<". "; 
		...
		if(s<=k) 
		{ 
			outfile_conjunto_independ<<"Independent Set ("<<n-s<<"): "; 
			for(j=0; j<cover.size(); j++) if(cover[j]==0) outfile_conjunto_independ<<j+1<<" "; 
			outfile_conjunto_independ<<endl; 
